...
export function spawn(
    command: string,
    args: child_process.SpawnOptionsWithoutStdio,
    ...
    spawnedProcess.on("error", (_err: unknown) => {
    ...
    spawnedProcess.on("close", (code: number, signal: unknown) => {
        if (err === null && code !== 0) {
...
export function spawnReg(
    args: child_process.SpawnOptionsWithoutStdio,
    callback: (arg0: Error, arg1: string) => unknown
) {
...
export function addToRegistry(queue: unknown[][], callback: () => unknown) {
    if (queue.length === 0) {
    ...
    args.unshift("add");
    args.push("/f");
    return spawnReg(args as child_process.SpawnOptionsWithoutStdio, () =>
        addToRegistry(queue, callback)
    );
}
