...
export function setSplashState(state: string) {
    splashState = state;
    if (
        splashWindow != null &&
        ...
        webContentsSend(
            splashWindow,
            "SPLASH_UPDATE_STATE",
            Object.assign({ status: lastStatus }, splashState)
        );
    }
}
...
export function initSplash(_startMinimized = false) {
    modulesListeners = {};
    splashState = {};
  ...
  addModulesListener(UPDATE_CHECK_FINISHED, ({ succeeded, updateCount, manualRequired }) => {
    ...
    ipcMain.default.on("UPDATED_QUOTES", (_event, quotes) =>
        cacheLatestQuotes(quotes)
    );
}
...
export function updateSplashState(event) {
    if (
        ...
        webContentsSend(
            splashWindow,
            "SPLASH_UPDATE_STATE",
            Object.assign({ status: event }, splashState)
        );
    }
}
...
function launchSplashWindow() {
    ...
    splashWindow.webContents.session.protocol.interceptFileProtocol(
        "http",
        (request, callback) => {
            callback(
                path.join(
